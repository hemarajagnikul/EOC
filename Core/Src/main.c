...
float time=0.05;
float Set_pt=10; //gpio1
float Current_position=45; //gpio2
float pos_P=2;   //main file
float pos_I=3;
float speed_P=1.5;
float speed_I=2;
float curr_P=1;
float curr_I=1;
float pos_loop,speed_loop,current_loop;
float Output[3];
float hall_vec[3];
float current_in = 36; //UART_Txn  //gpio3

float deriv;

int hall_out[3];
int hall_output; //optimize
int input_halls[3]={1,1,0};  //gpio4,5,6
...
HAL_Delay(300);
//HAL_USART_Transmit(&huart2, pData, Size);
  }
  return hall_out;
}
